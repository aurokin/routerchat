...
- [~] 1. Tool calling — wire layer + streaming accumulator landed. New types: `FunctionDefinition`, `FunctionTool`, `ToolChoice` (string-or-object union), `ToolCall`, `ToolCallDelta`. `OpenRouterTool = WebSearchTool | FunctionTool`; `OpenRouterMessage` accepts `tool_calls?`, `tool_call_id?`, `name?` for replay + tool-result messages; `ChatCompletionRequest` gains `tool_choice?` and `parallel_tool_calls?`; `ChatCompletionResponse.choices[0].message` gains `tool_calls?`. Request builder accepts `functionTools[]` + `toolChoice` + `parallelToolCalls`; user-supplied function tools land before the implicit web-search tool so any downstream tool-count cap truncates server-injected tools first. The streaming SSE parser now merges `delta.tool_calls[]` by `index` (taking first non-empty `id`/`type`/`name`, appending `arguments` string fragments) and surfaces the finalized `ToolCall[]` on the assistant message. Partial accumulations survive mid-stream cancellation as `finish_reason: "length"`. `ToolCall.function.arguments` is documented as a JSON-encoded string that callers MUST wrap in `try/catch` before parsing. 13 new tests cover argument merging across chunks, parallel calls by index, out-of-order indices, missing-id drop, late-arriving id, text+tool coexistence, terminator chunks without `delta.tool_calls`, and cancellation mid-flight. End-to-end UI (tool-call render, client-side execution loop, per-skill tool config) deferred to a follow-up workstream — `useSendMessage` does not yet thread `functionTools`.
